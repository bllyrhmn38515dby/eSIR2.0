...
async function fixDatabaseFinal() {
  try {
    console.log('🔧 Memperbaiki database secara final...');
    ...
    // Drop tabel yang memiliki foreign key constraints
    try {
      await connection.execute('DROP TABLE IF EXISTS notifications');
      console.log('✅ Tabel notifications dihapus');
    } catch (error) {
      console.log('⚠️ Tabel notifications tidak ada');
    }

    try {
      await connection.execute('DROP TABLE IF EXISTS referrals');
      console.log('✅ Tabel referrals dihapus');
    } catch (error) {
      console.log('⚠️ Tabel referrals tidak ada');
    }

    try {
      await connection.execute('DROP TABLE IF EXISTS tracking_logs');
      console.log('✅ Tabel tracking_logs dihapus');
    } catch (error) {
      console.log('⚠️ Tabel tracking_logs tidak ada');
    }

    try {
      await connection.execute('DROP TABLE IF EXISTS rujukan');
      console.log('✅ Tabel rujukan dihapus');
    } catch (error) {
      console.log('⚠️ Tabel rujukan tidak ada');
    }
    ...
    try {
      ...
      console.log('⚠️ Tabel users tidak ada');
    }
    ...
    `);
    console.log('✅ Tabel rujukan dibuat');

    // Insert admin user
    const bcrypt = require('bcryptjs');
    const hashedPassword = await bcrypt.hash('admin123', 10);
    await connection.execute(`
      INSERT INTO users (id, username, password, email, nama_lengkap, role_id) VALUES 
    ...
    await connection.execute(`
      INSERT INTO rujukan (nomor_rujukan, pasien_id, faskes_asal_id, faskes_tujuan_id, diagnosa, alasan_rujukan, user_id) VALUES 
    ...
    `);
    console.log('✅ Sample data rujukan dibuat');
